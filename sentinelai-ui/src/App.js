...
import EngineeringContextExplorer from './pages/EngineeringContextExplorer';
...
const navStyles = {
  ...
  contentWide: { maxWidth: 1200, margin: '32px auto', padding: '0 16px' },
};

// ── Existing RCA page — logic unchanged, EngineeringContextPanel added ─────
function RcaPage() {
    ...
    <div style={{ maxWidth: 760, margin: '0 auto', padding: 24, border: '1px solid #eee', borderRadius: 8, background: '#fff' }}>
      ...
      {result && (
        <div style={{ marginTop: 32, background: '#f9f9f9', padding: 16, borderRadius: 6 }}>
          ...
          {/* ── Engineering Context Used — new collapsible section ───── */}
...
function App() {
  ...
  // The Explorer page needs more horizontal space than RCA/ELK.
  const contentStyle = activePage === PAGES.context
    ? navStyles.contentWide
    : navStyles.content;
  ...
  return (
    <div style={navStyles.wrapper}>
      ...
      <nav style={navStyles.nav}>
        ...
        {/* ── NEW: Engineering Context Explorer tab ──────────────────── */}
        <button
          style={navStyles.tab(activePage === PAGES.context)}
          onClick={() => setActivePage(PAGES.context)}
        >
      ...
      <div style={contentStyle}>
        ...
        {activePage === PAGES.context && <EngineeringContextExplorer />}
      </div>
    </div>
  );
}
